...
window.addCheckmark = () => {
    ...
    checkDiv.innerText = "✔"; // เครื่องหมายติ๊กถูกสีดำ
...
// --- 3. ระบบจัดการ Element (ลาก/ย่อ/ลบ) ---
function setupInteract(el) {
    let x = 0, y = 0;
    const ratio = el.tagName === 'IMG' ? el.naturalWidth / el.naturalHeight : null;
    interact(el).draggable({
        listeners: { move(event) { x += event.dx; y += event.dy; event.target.style.transform = `translate(${x}px, ${y}px)`; } }
    }).resizable({
...
function selectElementFunc(el) {
    document.querySelectorAll('.draggable-img, .draggable-text').forEach(item => item.style.border = "1px dashed #3498db");
    selectedElement = el;
    selectedElement.style.border = "2px solid #ff0000";
}

function deleteImage(el) {
    if (el && confirm("ลบส่วนที่เลือกใช่ไหม?")) { el.remove(); selectedElement = null; }
}

document.addEventListener('keydown', (e) => {
    if ((e.key === "Delete" || e.key === "Backspace") && selectedElement && document.activeElement !== selectedElement) {
        e.preventDefault(); deleteImage(selectedElement);
    }
}, true);

// --- 4. ระบบไฟล์และ PDF Logic ---
document.getElementById('save-btn').addEventListener('click', async () => {
    if (!pdfDocBytes) return alert("ไม่มีไฟล์!");
    ...
    for (const el of elements) {
        ...
        if (el.dataset.type === "text") {
            page.drawText(el.innerText, { x: pX, y: pY + (5 * rY), size: parseFloat(window.getComputedStyle(el).fontSize) * rY, font: font, color: rgb(0,0,0) });
        } else {
...
// ส่วนโหลด PDF และเปลี่ยนหน้า (เหมือนเดิม)
...
document.getElementById('img-input').addEventListener('change', (e) => {
    ...
    reader.onload = (ev) => {
        const img = document.createElement('img'); img.src = ev.target.result; img.classList.add('draggable-img'); img.style.width = '150px'; img.dataset.page = currentPageNum;
...
async function renderPage(num) {
    ...
    document.querySelectorAll('.draggable-img, .draggable-text').forEach(el => el.style.display = parseInt(el.dataset.page) === num ? 'block' : 'none');
